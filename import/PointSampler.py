...
class PointSampler:
	...
	def gridReject(
		...
		min_dist = radius - radius * overlap
		grid_points = createMeshGrid(radius/2.0)
		...
		order = (order*3083+313)%neighbor_offsets.shape[0]
	...
	@staticmethod
	def zero( preset: PalettePreset, point_count: int, oklab_origin: list[int] = [0.5, 0.0, 0.0] ):
	...
	@staticmethod
	def grayscale(point_count: int = None, point_radius: float = None, minimum: float=0.0, maximum: float=1.0 ):
